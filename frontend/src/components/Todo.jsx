...
const Todo = () => {
  ...
  const { user, backendURL, loading, setLoading,navigate } = useYourContext();
  ...
  return (
    <div className="w-full flex flex-col items-center">
      <span onClick={()=>navigate("/")} className="mr-auto p-1 rounded bg-gradient-to-r from-purple-600 to-indigo-400 cursor-pointer transition transform active:scale-90 mb-3 sm:mb-0" title="Go Back"><IoMdArrowBack size={22}/></span>
      
      <h1 className="text-center pb-5 mb-8 text-2xl sm:text-3xl font-medium flex items-center gap-3">
      {user?.name&&<p className="bg-gradient-to-r from-purple-600 to-indigo-400 inline-block text-transparent bg-clip-text">{user?.name}</p>}
      ...
      <form onSubmit={submitHandler} className="w-full max-w-[500px] mx-auto">
        <div className="w-full flex items-center gap-2">
          ...
          <button
            type="submit"
            disabled={loading}
            className={`p-2 rounded-lg bg-gradient-to-r from-purple-600 to-indigo-400 text-white font-bold cursor-pointer transition transform active:scale-90 ${loading&&"opacity-60"}`}
            title="Add Todo"
          >
            {loading ? (
              <Loader />
        ...
        {todos?.length > 0 &&
          todos
            .map((t, i) => (
              ...
              >
                <p className="text-slate-100 text-[16px] font-medium max-h-full overflow-x-auto element">{t?.title}</p>
                ...
                <div className="flex items-center gap-3">
                 ...
                 <p className="text-xs text-slate-400">{new Date(t?.createdAt).toDateString()}</p>
                </div>
              </div>
            ))
            .reverse()}
